class VideoPlayer {
  ...
  inicializeCodec () {
    ...
    if (!codecSupported) {
      alert(`Seu browser não tem suporte ao codec: ${this.manifestJSON.codec}`)
      return
    }
  ...
  waitForQuestions () {
    ...
    if (!option) return
    // prevents show 2x+ modal on same second
  ...
  async nextChunk (data) {
    ...
    // update time on player, by current time
    this.selected = { ...selected, at: parseInt(this.videoElement.currentTime + selected.at) }
